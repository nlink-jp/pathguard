...
6. **費用は 1 回の呼び出しで有界、何もキャッシュしない。** 1 回の検査で、各場所自身の形とその上位を 1 回ずつ
   調べ、資格情報のディレクトリはリンクを探して 1 回ずつ一覧し、各形の上の階層を、長さに比例する時間で 1 回
   たどる。どのシステムも開けない長さ（4096 バイト、Windows では 32 KiB）のパスは、その前に拒む。Apple Silicon
   での実測: 現実的なパスで 1 回の検査が約 2 ms（`BenchmarkLocalCheck`）、上限いっぱいのパスで約 17 ms
   （`BenchmarkLocalCheckLongestPath`）。比較はメモリ上で行う。キャッシュは、作った後に現れた場所を見落とす。
