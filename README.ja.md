...
`Resolver` を持たない呼び出し箇所は、パッケージの関数を使う。方針はこのプロセスのホームディレクトリから
作る（ホームが分からなければ拒む）:
...
- **実体と名前、常に両方。** 実体（形とその上にある存在するディレクトリに対する `os.SameFile`）は、存在する
  ...
  を捕まえる。名前の比較は、まだ存在しない場所と、実体の番号を信用できないファイルシステムを覆う。
- **名前はディスクと同じやり方で同一視する。** APFS は名前を ASCII の小文字化ではなく Unicode のケース
  フォールディングで照合する。`id_rſa` で `id_rsa` が開き、ケルビン記号で `k` が、`.ﬆ` で `.st` が開く。
  名前の比較も同じやり方で同一視し、`ß` → `ss` やラテン文字の合字の展開も含める。
- **場所の中のリンクの行き先も守る。** `~/.ssh/config` が同期フォルダへのリンクなら、その指す先のファイルを
  そちらの名前で渡しても拒む。
...
- 資格情報のディレクトリの中のファイルへの別名のハードリンクや、秘密の写しは、Local の方針では検出しない。
- 行き先までたどるのは、場所の直下にあるリンクだけ。もっと深いところのリンク（`~/.ssh/keys/work → …`）は、
  リンク自身の位置は守られるが、行き先は守られない。
...
- 実体の番号が不安定なファイルシステムでは、実体の比較が衝突して正当なパスを拒むことがある。Unicode の
  正規化（`é` の合成形と分解形）は実体でしか照合しないので、まだ存在しない場所には効かない。
